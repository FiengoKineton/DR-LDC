...
def run_main(run_fn: Callable[..., Dict[str, Any]], gamma: float, *, FROM_DATA: bool = False, comp: bool = False, plot: bool = False) -> Dict[str, Any]:
    """
    ...
    """
    # Example placeholder:
    return run_fn(gamma=gamma, FROM_DATA=FROM_DATA, comp=comp, plot=plot)
...
def _evaluate_gamma_once(gamma: float,
    ...
    t0 = time.time()
    # 1) baseline MBD
    _ = run_main(run_fn,gamma=gamma, FROM_DATA=False, comp=False, plot=False)
    # 2) generate data with this gamma
    #make_data_openloop(gamma=gamma)
    # 3) DDD run
    _ = run_main(run_fn, gamma=gamma, FROM_DATA=True, comp=False, plot=False)
    # 4) final comparison (must return the 'report' dict)
    report = run_main(run_fn, gamma=gamma, FROM_DATA=True, comp=True, plot=False)
...
def opt_gamma(run_fn: Callable[..., Dict[str, Any]]):
