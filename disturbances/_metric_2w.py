...
class Metric2Wasserstein:
    ...
    def w2_gaussian_zero_mean(S1: np.ndarray, S2: np.ndarray) -> float:
        ...
        """
        S1 = Metric2Wasserstein._spd_correction(S1)
        S2 = Metric2Wasserstein._spd_correction(S2)
    ...
    def w2_gaussian(mu1: np.ndarray, S1: np.ndarray,
        ...
        w2_cov = Metric2Wasserstein.w2_gaussian_zero_mean(S1, S2)
    ...
    def empirical_mean_and_cov(w: np.ndarray,
        ...
        S = z.T @ z / denom
        S = Metric2Wasserstein._spd_correction(S)
        return mu, S
